...
import { fetchBarang, fetchCustomer } from "@/lib/api";
import { toast } from "sonner";

interface Barang {
  id: string;
  nama_barang: string;
  harga: number; // pastikan ada properti harga
}
...
export default function StockFormModal({ order, trigger, onSubmit }: Props) {
  ...
  // Handler untuk ubah select barang
  ...
  // Handler untuk ubah select customer
          ...
          >
            ...
            {barangList.map((b) => (
              <option key={b.id} value={b.id}>
                {b.nama_barang} - Rp{b.harga.toLocaleString()}
              </option>
            ))}
          </select>
